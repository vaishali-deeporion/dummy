...
// Server-specific configuration for deployment isolation
const SERVER_ID = process.env.SERVER_ID || 'default';
const DATA_DIR = process.env.DATA_DIR || './data/default';
const BACKSTOP_DATA_DIR = process.env.BACKSTOP_DATA_DIR || path.join(DATA_DIR, 'backstop_data');
const UPLOADS_DIR = process.env.UPLOADS_DIR || path.join(DATA_DIR, 'uploads');
const CHROME_USER_DATA_BASE = process.env.CHROME_USER_DATA_BASE || `/tmp/chrome-${SERVER_ID}`;

// Ensure directories exist
[DATA_DIR, BACKSTOP_DATA_DIR, UPLOADS_DIR].forEach(dir => {
...
app.use('/backstop_data', express.static(BACKSTOP_DATA_DIR));
...
function clearBackstopData() {
    const dirsToClean = [
        path.join(BACKSTOP_DATA_DIR, 'bitmaps_reference'),
        path.join(BACKSTOP_DATA_DIR, 'bitmaps_test'),
        path.join(BACKSTOP_DATA_DIR, 'html_report')
    ];
...
async function loadAndScrollPage(scenario) {
    const browser = await puppeteer.launch({
        ...
        args: [
            ...
            `--user-data-dir=${CHROME_USER_DATA_BASE}-${Date.now()}-${Math.random()}`
        ]
    });
...
function generateBackstopConfig(prodUrl, stagingUrl, routes) {
    ...
    return {
        ...
        scenarios,
        paths: {
            bitmaps_reference: path.join(BACKSTOP_DATA_DIR, 'bitmaps_reference'),
            bitmaps_test: path.join(BACKSTOP_DATA_DIR, 'bitmaps_test'),
            engine_scripts: path.join(BACKSTOP_DATA_DIR, 'engine_scripts'),
            html_report: path.join(BACKSTOP_DATA_DIR, 'html_report'),
            ci_report: path.join(BACKSTOP_DATA_DIR, 'ci_report')
        },
        report: ["browser"],
        engine: "puppeteer",
        engineOptions: {
...
app.post('/api/test', async (req, res) => {
    ...
    try {
        ...
        const configPath = path.join(BACKSTOP_DATA_DIR, 'backstop.config.js');
        fs.writeFileSync(configPath, configContent);
        
        // Create reference images
        io.emit('status', 'Creating reference images...');
        const reference = spawn('npx', ['backstop', 'reference', `--config=${configPath}`]);
        ...
        reference.on('close', async (code) => {
            if (code === 0) {
                ...
                const test = spawn('npx', ['backstop', 'test', `--config=${configPath}`]);
...
app.post('/api/test-excel', async (req, res) => {
    ...
    try {
        ...
        reference.on('close', (code) => {
            if (code !== 0) {
                isTestRunning = false;
                io.emit('error', 'BackstopJS reference creation failed');
                return;
            }
            
            io.emit('status', 'Reference images created successfully. Starting test...');
            const test = spawn('npx', ['backstop', 'test', `--config=${configPath}`]);
            ...
            test.on('close', (testCode) => {
                isTestRunning = false;
                if (testCode === 0) {
                    io.emit('status', 'Excel URL test completed - no differences found!');
                    io.emit('complete', '/backstop_data/html_report/index.html');
                } else if (testCode === 1) {
                    io.emit('status', 'Excel URL test completed - visual differences found!');
                    io.emit('complete', '/backstop_data/html_report/index.html');
                } else {
                    io.emit('error', 'BackstopJS test failed with unexpected code: ' + testCode);
                }
            });
        });
        
        // Send immediate response to client
        res.json({ message: 'Excel test process started' });
    } catch (err) {
        isTestRunning = false;
...
app.get('/api/results', (req, res) => {
    try {
        const configPath = path.join(BACKSTOP_DATA_DIR, 'html_report', 'config.js');
...
// API endpoint to reset test state
app.post('/api/reset', (req, res) => {
    isTestRunning = false;
    io.emit('status', 'Test state reset');
